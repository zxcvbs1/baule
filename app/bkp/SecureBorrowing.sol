...
contract SecureBorrowing is ReentrancyGuard, Ownable, Pausable {
    ...
    function _calculateReputationChange(uint256 amount) internal pure returns (int256) {
        if (amount == 0) return 0;
        int256 change = int256(amount / 100);
        return change == 0 ? int256(1) : change; // Mínimo 1 si amount > 0
    }
    ...
    function getItemInfo(bytes32 itemId) external view returns (ItemInfo memory) {
        return items[itemId];
    }
