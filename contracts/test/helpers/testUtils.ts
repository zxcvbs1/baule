...
import { parseEther, zeroAddress, stringToHex, keccak256, toHex, decodeEventLog } from "viem"; // Agrega stringToHex, etc. si son usadas por helpers aquí
import { expect } from "chai"; // Si verifyEvent se mueve aquí
import { loadFixture } from "@nomicfoundation/hardhat-toolbox-viem/network-helpers";

// Asegúrate de que el nombre del artefacto coincida con tu compilación
// Podrías necesitar importar ABIs si funciones aquí los usan directamente,
// aunque es mejor que la fixture devuelva las instancias del contrato.
...
export async function createTestItem(secureBorrowingInstance, options = {}) {
    ...
    await secureBorrowingInstance.write.listItem([
      itemId, fee, deposit, metadataHash, minBorrowerReputation
    ]);
...
export async function setupBorrowingTransactionAndGetId(
  ...
  // Crear ítem
  const { itemId, fee, deposit } = await createTestItem(
    secureBorrowingContract, 
 ...
 */
export async function initiateDisputeAndGetReceipt(
  secureBorrowingContract,
  ownerLedger,
  transactionId,
  publicClient
...
export async function verifyDisputeState(
  ...
  const eventLog = receipt.logs.find(log => 
    ...
    log.address.toLowerCase() === arbitrationContract.address.toLowerCase()
  );
  expect(eventLog, `Event ${eventSignature} not found`).to.exist;
 ...
 */
export async function setupExpiredVotingPeriod(
  activeDisputeResult,
  ownerArbitration
) {
 ...
 */
export async function hasArbitratorVoted(arbitrationContract: any, transactionId: bigint, arbitratorAddress: string): Promise<boolean> {
  try {
    // Llama a la nueva función getter en el contrato
    const hasVotedStatus = await arbitrationContract.read.getArbitratorHasVoted([
      transactionId,
      arbitratorAddress
    ]);
...
export async function getArbitratorVoteDetails(arbitrationContract, transactionId, arbitratorAddress) {
  try {
    const voteDetails = await arbitrationContract.read.getArbitratorVote([
      transactionId, arbitratorAddress
    ]);
